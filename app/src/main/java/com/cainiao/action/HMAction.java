                            ...
                            if (tbData.size() > 0) {    //获取买号成功
                                ...
                                for (int i = 0, len = tbData.size(); i < len; i++) {
                                    if ("//cdn.heima911.com/binary/taobao_bind_ok.png".equals(tbData.get(i).select(".item-media").select("img").attr("src"))) {
                                        ...
                                        count++;
                                        list.add(new BuyerNum(href, tbData.get(i).select(".item-title-row").select(".item-title").text()));
                                    }
                                }
        ...
        if (mParams.getType().equals("1")) {
                        ...
                        public void onSuccess(Response<String> response) {
                            try {
                                ...
                                if (obj.getInteger("code") == 0) {
                                    ...
                                    receiveSuccess(String.format(MyApp.getContext().getString(R.string.KSHG_AW_tips), mPlatform.getName()) + ",店铺名:" + obj.getJSONObject("result").getString("shop_name"), R.raw.heima, 3000);
            ...
            HttpClient.getInstance().post("/mm/task/claim_task?task_type=DIANFU", mPlatform.getHost())
                    ...
                    .execute(new StringCallback() {
                        ...
                        public void onSuccess(Response<String> response) {
                            try {
                                HttpClient.getInstance().get("/mm/task/task_list?d_index=0&d_per_page=30&b_type=DIANFU&model=1", mPlatform.getHost())
                                        .params("user_name", mParams.getAccount())
                                        .params("password", mParams.getPassword())
                                        .headers("Referer", "http://www.heima911.com/mm/user")
                                        .headers("X-Requested-With", "XMLHttpRequest")
                                            ...
                                            public void onSuccess(Response<String> response) {
                                                try {
                                                    if (!TextUtils.isEmpty(response.body())){
                                                        sendLog("接单成功,可前往平台做单");
                                                        receiveSuccess(String.format(MyApp.getContext().getString(R.string.KSHG_AW_tips), mPlatform.getName()), R.raw.heima, 3000);
                                                        addTask(mPlatform.getName());
                                                        updateStatus(mPlatform, Const.KSHG_AW); //接单成功的状态
                                                        isStart = false;
                                                    }else{
                                                        sendLog("暂时没有任务,请稍后重试");
                                                    }
                                                } catch (Exception e) {
                                                    sendLog("接单异常！");
                                                    stop();
